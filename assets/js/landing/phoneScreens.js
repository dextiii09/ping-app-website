...
// campaign, talent applies, the brand taps Connect on an applicant, a chat
...
// One applicant in the brand's list, with Pass and Connect.
function applicant(photo, info, { tap = false } = {}) {
  return `
    <div class="ps-row">
      <span class="ps-row-av" style="background-image:url('${photo}')"></span>
      <div class="ps-row-info">
        <b>${info.name} <i class="ph-fill ph-seal-check"></i></b>
        <span class="ps-row-meta"><i class="ph-fill ${info.icon}"></i> ${info.type} · ${info.place}</span>
        <span class="ps-row-stats">${info.stats}</span>
        <span class="ps-row-quote">“${info.pitch}”</span>
      </div>
      <div class="ps-row-actions">
        <span class="ps-pass">Pass</span>
        <span class="ps-connect">
          <span class="ps-connect-a"><i class="ph-bold ph-user-plus"></i> Connect</span>
          <span class="ps-connect-b"><i class="ph-bold ph-check"></i> Connected</span>
          ${tap ? '<span class="ps-ripple"></span><span class="ps-ripple"></span><span class="ps-tap"></span>' : ''}
        </span>
      </div>
    </div>`;
}
...
const SCREENS = [
  ...
  // 2 - The brand taps Connect on an applicant
  () => `
    <div class="ps ps-pick">
      ...
      <div class="ps-list">
        ${applicant(PHOTOS.simran, SIMRAN, { tap: true })}
        ${applicant(PHOTOS.dj, DJ)}
        ${applicant(PHOTOS.zoya, ZOYA)}
      </div>
      <div class="ps-toast"><i class="ph-fill ph-chats-circle"></i> Connected with Simran · chat open</div>
    </div>`,
...
export function buildScreenTimeline(gsap, el, index) {
  ...
  if (index === 2) {
    const rows = qa('.ps-row');
    const first = rows[0];
    const btn = first.querySelector('.ps-connect');
    const tap = q('.ps-tap');
    const dots = qa('.ps-slots i');
    tl.set(dots[1], { className: '' }, 0)
      .set(btn, { className: 'ps-connect' }, 0)
      .set(first, { className: 'ps-row' }, 0)
      .from(rows, { y: 24, opacity: 0, stagger: 0.12, duration: 0.6, ease: 'expo.out' }, 0.1)
      .fromTo(tap, { opacity: 0, scale: 1.6, x: 60, y: 90 }, { opacity: 1, scale: 1, x: 0, y: 0, duration: 0.7, ease: 'power3.out' }, 1.0)
      .to(tap, { scale: 0.7, duration: 0.12, yoyo: true, repeat: 1, ease: 'power2.inOut' }, 1.75)
      .to(btn, { scale: 0.95, duration: 0.12, yoyo: true, repeat: 1, ease: 'power2.inOut' }, 1.75)
      .set(btn, { className: 'ps-connect is-done' }, 1.87)
      .set(first, { className: 'ps-row is-connected' }, 1.87)
      .fromTo(first.querySelectorAll('.ps-ripple'), { scale: 0.8, opacity: 0.9 }, { scale: 1.6, opacity: 0, duration: 1.0, stagger: 0.16, ease: 'power2.out' }, 1.87)
      .to(tap, { opacity: 0, duration: 0.3 }, 2.0)
      .set(dots[1], { className: 'is-on' }, 2.1)
      .fromTo(dots[1], { scale: 0.4 }, { scale: 1, duration: 0.5, ease: 'back.out(3)' }, 2.1)
      .fromTo(q('.ps-toast'), { y: -60, opacity: 0 }, { y: 0, opacity: 1, duration: 0.7, ease: 'expo.out' }, 2.2)
